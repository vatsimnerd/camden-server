use super::spatial::{PointObject, RectObject};
use crate::{
  config::Config,
  fixed::{
    data::FixedData,
...
impl Manager {
  pub async fn new(cfg: Config) -> Self {
    ...
    let tracks = res.ok().map(|ts| RwLock::new(ts));
  ...
  pub async fn get_firs(&self, env: &AABB<Point>) -> Vec<FIR> {
    ...
    for po in firs2d.locate_in_envelope(env) {
  ...
  pub async fn get_pilot_by_callsign(&self, callsign: &str) -> Option<Pilot> {
    self.pilots.read().await.get(callsign).map(|p| p.clone())
  }
